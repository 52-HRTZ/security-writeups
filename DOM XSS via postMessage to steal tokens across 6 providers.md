...
The page itself looked almost empty. There was no UI, no login form, and nothing that immediately stood out.
...
The page accepted `postMessage` events from its opener without validating the sender's origin.
...
Instead of stopping at `alert(1)`, I wanted to abuse the OAuth flow
itself.

1.  Embed the OAuth handler.
2.  Trigger the vulnerable `postMessage`.
3.  Execute JavaScript.
4.  Open the OAuth popup.
5.  Wait for authentication.
6.  Read the access token.
7.  Exfiltrate the token.
